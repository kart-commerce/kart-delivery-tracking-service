...
RUN --mount=type=cache,target=/root/.nuget/packages,id=nuget-packages \
    dotnet restore src/Api/KartDeliveryTrackingService.Api.csproj
...
RUN --mount=type=cache,target=/root/.nuget/packages,id=nuget-packages \
    dotnet publish src/Api/KartDeliveryTrackingService.Api.csproj -c Release -o /app/publish --no-restore
